        ...
        <div class="max-w-7xl mx-auto px-4 py-8">
            ...
            <div class="bg-yellow-50 border-l-4 border-yellow-400 p-6 mb-6">
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <h3 class="text-lg font-medium text-yellow-800">Development Mode</h3>
                        <div class="mt-2 text-sm text-yellow-700">
                            <p class="mb-2">
                                <strong>The CMS upload functionality requires production deployment.</strong>
                            </p>
                            <p class="mb-2">
                                This interface is for demonstration purposes only. To use the CMS:
                            </p>
                            <ol class="list-decimal list-inside space-y-1 mb-3">
                                <li>Deploy the application to Cloudflare Pages</li>
                                <li>Create a D1 database and run migrations</li>
                                <li>Create an R2 bucket for file storage</li>
                                <li>Configure bindings in wrangler.jsonc</li>
                            </ol>
                            <a href="https://github.com/bchiyangwa9/igcse-study-buddy/blob/main/CMS_DEPLOYMENT_GUIDE.md" 
                               target="_blank"
                               class="inline-flex items-center text-yellow-800 hover:text-yellow-900 font-medium">
        ...
        document.getElementById('infographic-form').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    let errorMsg = 'Upload failed: ' + result.error;
                    if (result.info) {
                        errorMsg += '\\n\\n' + result.info;
                    }
                    if (result.deployment_guide) {
                        errorMsg += '\\n\\nPlease see ' + result.deployment_guide + ' for deployment instructions.';
                    }
                    alert(errorMsg);
                }
            } catch (error) {
...
app.post('/api/admin/resources/infographic', async (c) => {
  try {
    ...
    // Check if R2 and D1 are available (production mode)
    if (!c.env.LEARNING_RESOURCES || !c.env.DB) {
      return c.json({ 
        error: 'CMS is only available in production. Please deploy to Cloudflare Pages first.',
        info: 'R2 bucket and D1 database must be configured in production environment.',
        deployment_guide: '/CMS_DEPLOYMENT_GUIDE.md'
      }, 503);
    }
